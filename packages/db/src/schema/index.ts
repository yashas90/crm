...
export const agentDevices = pgTable(
  "agent_devices",
  {
    ...
    queuedOfflinePingCount: integer("queued_offline_ping_count").notNull().default(0),
    permissionDeniedAt: timestamp("permission_denied_at", { withTimezone: true }),
    permissionDeniedCount: integer("permission_denied_count").notNull().default(0),
    batteryOptimizationIgnored: boolean("battery_optimization_ignored"),
    deviceStatus: text("device_status").notNull().default("UNKNOWN"),
    healthStatus: text("health_status").notNull().default("UNKNOWN"),
    /** Spec enum: active | paused | stale | offline — STALE only if likely uninstalled. */
...
export const trackingSettings = pgTable(
  "tracking_settings",
  {
    ...
    possibleUninstallMinutes: integer("possible_uninstall_minutes").notNull().default(1440),
