...
const aapt = androidHome
  ? path.join(androidHome, "build-tools", "35.0.0", "aapt")
  : "aapt";
...
try {
  execFileSync("unzip", ["-q", "-o", apk, "assets/index.android.bundle", "assets/app.config", "-d", tmp], {
    stdio: "ignore",
  });
} catch {
...
must(bundle.includes(Buffer.from("crm-production-e81d.up.railway.app")), "bundle targets production API");
...
if (fs.existsSync(configPath)) {
  ...
  must(config.extra?.apiUrl?.includes("crm-production-e81d"), "app.config extra.apiUrl is production");
  must(typeof config.version === "string" && /^\d+\.\d+\.\d+$/.test(config.version), `version ${config.version}`);
}
