...
/**
 * Tracks when the user left for the native dialer and fires `onReturn` with elapsed
 * seconds when the screen regains focus after a plausible call.
 */
export function useReturnFromDialerLog(onReturn: (elapsedSeconds: number) => void) {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (elapsed >= MIN_AUTO_LOG_MS && elapsed <= MAX_AUTO_LOG_MS) {
        ...
        callStartTsRef.current = undefined;
        onReturn(Math.max(1, Math.round(elapsed / 1000)));
      }
    }, [onReturn]),
  );
