...
- `"version": "0.0.11"` (or higher — not `0.0.0`)
- `"deployMarker": "railway-ipv6-health-2026-08-21"` (or the current marker in code)
...
3. **Settings → Build → Builder must be Dockerfile**
   ...
   - Success looks like: `FROM node:22-bookworm-slim` and `Baked deploy identity { version: '0.0.11', ... }`.
...
If Deploy logs show `@propninja/api@0.0.0 start`, that container is **not** current `main` (which is `0.0.11+`). Do not treat it as a successful upgrade — check Deployments → **Active** commit SHA and remove any stuck old deployment still bound to the public domain.

In Build logs you must see `Baked deploy identity { version: '0.0.11', deployMarker: 'railway-ipv6-health-2026-08-21', ... }`. If that line is missing, the Dockerfile bake step did not run.
